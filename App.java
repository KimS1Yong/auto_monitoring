...
//                    HttpResponse.BodyHandlers.ofString());
            ...
            logger.info("제목 목록 생성");
            String imageResponse = getDataFromAPI("image", keyword, display, start, sort);
            imageLink = imageResponse.split("link\":\"")[1].split("\",")[0].split("\\?")[0].replace("\\", "");
            logger.info(imageLink);
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(imageLink))
                    .build();
            String[] temp2 =imageLink.split("\\.");
            Path path = Path.of("%d_%s.%s".formatted(new Date().getTime(), keyword, temp2[temp2.length-1]));
